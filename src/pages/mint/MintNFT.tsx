...
type PublicMintNFTProps = {
  ...
  whiteListMintProof?: any
}

const MintNFT = ({ isPublicMint, whiteListMintProof }: PublicMintNFTProps) => {
  ...
  // 定义 WhitelistArgs 的结构和编码方式
    ...
    const whitelistMint = useCallback(
      async () => {
        ...
        try {
          // 编码 WhitelistArgs
          const args = encodeWhitelistArgs(whiteListMintProof)
          const mintTx = new Transaction()
          const mintKey = Keypair.generate()
      ...
      [publicKey, sendTransaction, connection, whiteListMintProof]
    )

  return (
    <button onClick={isPublicMint ? mintNFT : whitelistMint} disabled={(!publicKey || minting) || (!isPublicMint && !whiteListMintProof)}>
